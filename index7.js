...
(async () => {
  const listingUrl = 'https://www.airbnb.com/rooms/1528429546779826600?source_impression_id=p3_1769407479_P3fHQyyvLptlmU-Z&modal=PHOTO_TOUR_SCROLLABLE';
  const baseName = '10 BR Charming colonial-style mansion in Getsemaní with a large swimming pool - Cartagena Villas Colombia';
  ...
  if (!fs.existsSync('10BRCharmingcolonial')) fs.mkdirSync('10BRCharmingcolonial');
  ...
  for (const imgUrl of uniqueImages) {
    try {
      ...
      fs.writeFileSync(`10BRCharmingcolonial/${filename}`, buffer);
