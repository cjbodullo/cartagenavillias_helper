...
(async () => {
  const listingUrl = 'https://www.airbnb.com/rooms/53113312?source_impression_id=p3_1769408202_P3CgN70dV_vtBETg&modal=PHOTO_TOUR_SCROLLABLE';
  const baseName = '5 BR Luxury 5-bedroom house with private pool - rooftop terrace - Cartagena City Center - Cartagena Villas Colombia';
  ...
  if (!fs.existsSync('5BRterrace')) fs.mkdirSync('5BRterrace');
  ...
  for (const imgUrl of uniqueImages) {
    try {
      ...
      fs.writeFileSync(`5BRterrace/${filename}`, buffer);
      console.log(`✔ Saved: ${filename}`);
      index++;
    } catch (e) {
      console.warn(`⚠ Failed: ${imgUrl} - ${e}`);
    }
  }
